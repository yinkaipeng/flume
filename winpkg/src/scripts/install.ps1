...
###
### Install script that can be used to install Flume as a Single-Node cluster.
...
###   Flume services as Windows services.
...
function Main( $scriptDir )
{
    ...
    ###
    if ( $ENV:IS_FLUME -eq "yes" ) {
      $roles = "flumeagent"
    }
    Write-Log "Roles are $roles"
    Install "flume" $nodeInstallRoot $serviceCredential $roles

    Write-Log "Installation of Flume completed successfully"
}

try
{
    $scriptDir = Resolve-Path (Split-Path $MyInvocation.MyCommand.Path)
    $utilsModule = Import-Module -Name "$scriptDir\..\resources\Winpkg.Utils.psm1" -ArgumentList ("FLUME") -PassThru
